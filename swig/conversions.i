...
#include <utility>
...
#if defined(SWIGMATLAB)
        if (!mxIsScalar(input) || !mxIsNumeric(input)) return false;
        return true;
#elif defined(SWIGPYTHON)
        if (!PyLong_Check((PyObject *)input)) return false;
        return true;
#endif
    }
    ...
    bool is_real(const LangObject *input) {
        ...
        return true;
#elif defined(SWIGPYTHON)
        if (!PyFloat_Check((PyObject *)input)) return false;
        return true;
#endif
    }
...
#if defined(SWIGMATLAB)
        if (!mxIsNumeric(input)) return false;
        mwSize nb_dims = mxGetNumberOfDimensions(input);
        if (nb_dims != 2) return false;
        return true;
#elif defined(SWIGPYTHON)
        if (!PyArray_Check(input)) return false;
        int nb_dims = PyArray_NDIM((PyArrayObject *)input);
        if (nb_dims < 1 || nb_dims > 2) return false;
        return true;
#endif
    }

    int numRows(const LangObject *input) {
        if (!is_matrix(input)) throw std::invalid_argument("Input is not a valid matrix.");
#if defined(SWIGMATLAB)
    ...
    bool is_matrix(const LangObject *input, const int_t nb_rows, const int_t nb_columns) {
        if (!is_matrix(input)) return false;
        if (nb_rows != numRows(input) || nb_columns != numColumns(input)) return false;
        return true;
    }

    double *asDoublePointer(LangObject * input) {
        if (!is_matrix(input)) throw std::invalid_argument("Input is not of a valid matrix type.");
#if defined(SWIGMATLAB)
    ...
    LangObject *new_matrix(std::pair<int, int> dimensions, const T *data) {
        ...
        for (int_t i = 0; i < nb_rows * nb_cols; i++) new_array[i] = data[i];
        ...
        if (matrix == NULL) throw std::runtime_error("Something went wrong while copying array");
        return matrix;
#endif
    }
...
#if defined(SWIGMATLAB)
        if (!mxIsStruct(object)) return false;
        if (mxGetNumberOfElements(object) != 1) return false;
#elif defined(SWIGPYTHON)
        if (!PyDict_Check(object)) return false;
#endif
        return true;
    }
...
#if defined(SWIGMATLAB)
        if (mxGetField(map, 0, key) == NULL) return false;
#elif defined(SWIGPYTHON)
        if (PyDict_GetItemString((PyObject *)map, key) == NULL) return false;
#endif
        return true;
    }
    ...
    LangObject *new_sequence_of_arrays(const int_t length) {
        ...
        if (pClass) Py_INCREF(pClass);
        PyObject *sequence = NULL;
        if (PyCallable_Check(pClass)) {
            PyObject *args = PyTuple_New(1);
            PyObject *list = PyList_New(length);
            for (int_t index = 0; index < length; index++)
                PyList_SetItem(list, index,
                               PyLong_FromLong((long)index));  // fill list with dummies
            Py_INCREF(list);
        ...
        if (sequence == NULL) {
            ...
            snprintf(err_msg, sizeof(err_msg),
                     "Something went wrong during construction of %s "
                     "with length %d",
                     LANG_SEQUENCE_NAME, length);
    ...
    template <typename T>
    LangObject *new_sequence_from(T * *data, const int_t length, const int_t *nb_rows,
                                  const int_t *nb_columns) {
    ...
    template <typename T>
    LangObject *new_sequence_from(T * *data, const int_t length, const int_t *nb_elems) {
        int_t *nb_columns = (int_t *)calloc(length, sizeof(int_t));
        for (int_t i = 0; i < length; i++) nb_columns[i] = 1;
        ...
        for (int_t i = 1; i < length; i++) {
            if (nb_rows[i] != rows || nb_cols[i] != cols) {
                throw std::invalid_argument(
                    "If just given one matrix, dimensions for all stages "
                    "must be equal");
                return false;
            }
        }
    ...
    void copy_from(const LangObject *matrix, T *data, const int_t nb_elems) {
        ...
        if (PyArray_TYPE((PyArrayObject *)matrix) == get_numeric_type<int_t>()) {
            ...
            throw std::invalid_argument(
                "Only matrices with integer numbers or double "
    ...
    void fill_array_from(const LangObject *input, T **array, const int_t length,
        ...
        if (is_matrix(input) && dimensions_match(input, nb_rows, nb_columns, length)) {
            ...
            char err_msg[MAX_STR_LEN];
            snprintf(err_msg, sizeof(err_msg), "Expected %s or %s as input", LANG_SEQUENCE_NAME,
                     LANG_MATRIX_NAME);
            throw std::invalid_argument(err_msg);
        }
    }
    ...
    void fill_array_from(const LangObject *input, T **array, const int_t length) {
        int_t nb[length];
        for (int_t i = 0; i < length; i++) nb[i] = 1;
    ...
    LangObject *new_output_tuple(int_t num_fields, const char **field_names, LangObject **content) {
        ...
        // Try loading Python module into global variable
        if (copy_module == NULL) copy_module = PyImport_Import(PyString_FromString("copy"));
        // Check if loading was succesful
        if (copy_module == NULL)
            SWIG_Error(SWIG_RuntimeError,
                       "Something went wrong when importing Python module 'copy'");
    ...
    LangObject *new_ocp_nlp_function_output_tuple(LangObject * y, LangObject * jac_y,
                                                  LangObject * hess_y) {
    ...
    void fill_array_from(const LangObject *input, int_t *array, const int_t length) {
        if (is_integer(input)) {
            ...
            for (int_t i = 0; i < length; i++) array[i] = number;
        } else if (is_sequence(input, length)) {
            fill_int_array_from(input, array, length);
        } else {
            char err_msg[MAX_STR_LEN];
            snprintf(err_msg, sizeof(err_msg), "Expected scalar or %s of length %d",
                     LANG_SEQUENCE_NAME, length);
            throw std::invalid_argument(err_msg);
        }
    }
