...
#include "acados/utils/print.h"
      ...
      bool is_valid_ocp_dimensions_map(const LangObject *input){if (!is_map(input)) return false;
...
LangObject *ocp_qp_output(const ocp_qp_in *in, const ocp_qp_out *out) {
    ...
    for (int_t i = 0; i <= dims->N; i++) {
        ...
        for (int_t j = 0; j < dims->nu[i]; j++) controls_copy[i][j] = BLASFEO_DVECEL(out->ux, j);
    }
...
% include "acados_cpp/ocp_qp_solution.hpp" % ignore extract;
