...
static const int CASADI_PREFIX(s2)[] = {5, 1, 0, 5, 0, 1, 2, 3, 4};
#define s2 CASADI_PREFIX(s2)
/* adjFun */
int vde_adj_pendulum(void* mem, const real_t** arg, real_t** res, int* iw, real_t* w) {
...
int vde_adj_pendulum_init(int* n_in, int* n_out, int* n_int, int* n_real) {
  if (n_in) *n_in = 3;
  if (n_out) *n_out = 1;
  if (n_int) *n_int = 0;
  if (n_real) *n_real = 0;
...
int vde_adj_pendulum_alloc(void** mem, const int* idata, const double* rdata) {
  if (mem) *mem = 0;
  (void)idata;
  (void)rdata;
  return 0;
...
int vde_adj_pendulum_free(void* mem) {
  (void)mem;
  return 0;
}

int vde_adj_pendulum_sparsity(int i, int *nrow, int *ncol, const int **colind, const int **row) {
  const int* s;
  switch (i) {
    case 0:
    case 1:
...
int vde_adj_pendulum_work(int *sz_arg, int* sz_res, int *sz_iw, int *sz_w) {
  if (sz_arg) *sz_arg = 3;
  if (sz_res) *sz_res = 1;
  if (sz_iw) *sz_iw = 0;
  if (sz_w) *sz_w = 21;
