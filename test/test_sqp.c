...
void print_states_controls(real_t *w) {
    int_t   N = NNN;
    printf("node\tx\t\t\t\tu\n");
    for (int_t i = 0; i < N; i++) {
...
int main() {
    ...
    int_t   max_iters       = 1;           // Number of SQP iterations
    ...
    for (int_t iter = 0; iter < max_iters; iter++) {
        ...
        for (int_t i = 0; i < N; i++) {
            ...
            for (int_t j = 0; j < nx[i]; j++) {
                qp_in.q[i][j] = Q[j*(nx[i]+1)]*(w[i*(NX+NU)+j]-xref[j]);
            }
            for (int_t j = 0; j < nu[i]; j++) {
                qp_in.r[i][j] = R[j*(nx[i]+1)]*(w[i*(NX+NU)+NX+j]-uref[j]);
            }
        ...
        for (int_t i = 0; i < N; i++) {
            for (int_t j = 0; j < NX; j++) qp_out.x[i][j] = 0;
            for (int_t j = 0; j < NU; j++) qp_out.u[i][j] = 0;
        }
        for (int_t j = 0; j < NX; j++) qp_out.x[N][j] = 0;
