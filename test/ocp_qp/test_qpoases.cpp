...
std::vector<std::string> constraints = {"UNCONSTRAINED", "ONLY_AFFINE", "ONLY_BOUNDS", "CONSTRAINED"};
...
TEST_CASE("Solve random OCP_QP", "[qpOASES]") {
    ...
    for (std::string constraint : constraints) {
        for (std::string scenario : scenarios) {
            ...
            // std::cout << "Solving with qpOASES scenario: " + name_scenario + " with constraints: " + constraint << std::endl;
