...
void set_N2(std::string const &inString, void *opts, int N2, int N)
{
    ...
    {
        option_found = set_option_int(opts, "sparse_hpipm.pcond_N2", N2);
        REQUIRE(option_found == true);
    }

    if (inString == "SPARSE_HPMPC")
    {
        option_found = set_option_int(opts, "hpmpc.pcond_N2", N2);
        REQUIRE(option_found == true);
    }

    if (inString == "SPARSE_QPDUNES")
    {
        option_found = set_option_int(opts, "qpdunes.pcond_N2", N2);
        REQUIRE(option_found == true);
        if (N2 == N)
        {
    ...
    {
        option_found = set_option_int(opts, "sparse_ooqp.pcond_N2", N2);
        REQUIRE(option_found == true);
    }

    if (inString == "SPARSE_OSQP")
    {
        option_found = set_option_int(opts, "sparse_osqp.pcond_N2", N2);
        REQUIRE(option_found == true);
    }
}
