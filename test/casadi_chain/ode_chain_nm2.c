...
#else /* CODEGEN_PREFIX */
  #define CASADI_PREFIX(ID) ode_chain_nm2_ ## ID
...
/* vdeFun */
int ode_chain_nm2(void* mem, const real_t** arg, real_t** res, int* iw, real_t* w) {
    mem = 0; iw = 0; w = 0;
    mem += 0; iw += 0; w += 0;
...
int ode_chain_nm2_init(int* n_in, int* n_out, int* n_int, int* n_real) {
  if (n_in) *n_in = 4;
  if (n_out) *n_out = 3;
  if (n_int) *n_int = 0;
  if (n_real) *n_real = 0;
...
int ode_chain_nm2_alloc(void** mem, const int* idata, const double* rdata) {
  if (mem) *mem = 0;
  (void)idata;
  (void)rdata;
  return 0;
...
int ode_chain_nm2_free(void* mem) {
  (void)mem;
  return 0;
}

int ode_chain_nm2_sparsity(int i, int *nrow, int *ncol, const int **colind, const int **row) {
  const int* s;
  switch (i) {
    case 0:
    case 4:
...
int ode_chain_nm2_work(int *sz_arg, int* sz_res, int *sz_iw, int *sz_w) {
  if (sz_arg) *sz_arg = 4;
  if (sz_res) *sz_res = 3;
  if (sz_iw) *sz_iw = 0;
  if (sz_w) *sz_w = 3;
