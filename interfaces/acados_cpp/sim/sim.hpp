...
#ifndef INTERFACES_ACADOS_CPP_SIM_SIM_HPP_
#define INTERFACES_ACADOS_CPP_SIM_SIM_HPP_
...
class integrator
{
 public:
    ...
    integrator(int N, int nx, int nu, int ng = 0, int nh = 0, int ns = 0);

    ~integrator();

    void initialize_solver(std::string solver_name, std::map<std::string, option_t *> options = {});

    integrator_solution solve(std::vector<double> x_guess = {}, std::vector<double> u_guess = {});
 ...
 private:
    ...
    std::unique_ptr<integrator_in> nlp_;

    std::shared_ptr<integrator_dims> dims_;

    std::unique_ptr<integrator_solver_config> config_;

    std::unique_ptr<integrator_solver> solver_;
    ...
    std::shared_ptr<integrator_out> result_;

    integrator_solver_plan *plan_;
...
#endif  // INTERFACES_ACADOS_CPP_SIM_SIM_HPP_
