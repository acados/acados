...
bool
are_valid_ocp_dimensions(const map<string, vector<uint>>& dimensions,
...
create_ocp_qp_dimensions_ptr(const map<string, vector<uint>>& dims) {
...
create_ocp_nlp_dimensions_ptr(const map<string, vector<uint>>& dims) {
    ...
    vector<uint> nbu = dims.count("nbu") ? dims.at("nbu") : vector<uint>(N+1, 0);
    vector<uint> ng = dims.count("ng") ? dims.at("ng") : vector<uint>(N+1, 0);
    vector<uint> nh = dims.count("nh") ? dims.at("nh") : vector<uint>(N+1, 0);
    vector<uint> ns = dims.count("ns") ? dims.at("ns") : vector<uint>(N+1, 0);
    
    // calculate nv
    std::transform(nx.begin(), nx.end(), nu.begin(), nv.begin(), std::plus<uint>());
    std::transform(nv.begin(), nv.end(), ns.begin(), nv.begin(), std::plus<uint>());

    // calculate ni
    std::transform(nbx.begin(), nbx.end(), nbu.begin(), ni.begin(), std::plus<uint>());
    std::transform(ni.begin(), ni.end(), ng.begin(), ni.begin(), std::plus<uint>());
    std::transform(ni.begin(), ni.end(), nh.begin(), ni.begin(), std::plus<uint>());
