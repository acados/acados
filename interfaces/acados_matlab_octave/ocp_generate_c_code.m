function ocp_generate_c_code(obj)
        ...
        if ~strcmp( obj.model_struct.cost_type, 'linear_ls' ) || ...
           ~strcmp( obj.model_struct.cost_type_e, 'linear_ls' )
            error('mex templating does only support linear_ls cost for now. Got cost_type: %s, cost_type_e: %s.\nNotice that it might still be possible to solve the OCP from MATLAB.', obj.model_struct.cost_type, obj.model_struct.cost_type_e);
            % TODO: add
            % nonlinear least-squares
            % external cost
        elseif ~strcmp( obj.opts_struct.nlp_solver_exact_hessian, 'false')
            error('mex templating does only support nlp_solver_exact_hessian = "false", i.e. Gauss-Newton Hessian approximation for now. Got nlp_solver_exact_hessian = "%s"\n. Notice that it might still be possible to solve the OCP from MATLAB.', obj.opts_struct.nlp_solver_exact_hessian);
