	...
	methods
		...
		function obj = acados_ocp(model, opts)
			...
			if (strcmp(obj.opts_struct.codgen_model, 'true'))
				...
				if (strcmp(obj.model_struct.constr_type, 'bgh') && (isfield(obj.model_struct, 'expr_h') || isfield(obj.model_struct, 'expr_h_e')))
					% generate c for function and derivatives using casadi
				...
				if (strcmp(obj.model_struct.cost_type, 'nonlinear_ls') || strcmp(obj.model_struct.cost_e_type, 'nonlinear_ls'))
					% generate c for function and derivatives using casadi
					generate_c_code_nonlinear_least_squares(obj.model_struct, obj.opts_struct);
					% sources list
					if isfield(obj.model_struct, 'expr_y')
						c_sources = [c_sources, 'ocp_model_y_fun_jac_ut_xt.c '];
					end
					if isfield(obj.model_struct, 'expr_y_e')
