...
% cost
    ...
    &&&\underline{h}\initial \leq h\initial(x(0), u(0), z(0), p(0)) + J_{\textrm{sh}}\initial \, s\lowerh\initial, &&\label{eq:constraints:path:lower_bounds:start}\\
    &&& h\initial(x(0), u(0),  z(0), p(0)) - J\ind{sh}\initial\, s\ind{u,h}\initial \leq \overline{h}\initial, &&\label{eq:constraints:path:upper_bounds:start}\\
    &&&\mathComment{Dynamics, see section \ref{sec:dynamics}}\nonumber\\
    %% dynamics
    &&& f\ind{impl}(x(t), \dot{x}(t), u(t), z(t), t, p(t)) = 0, &&\quad t \in [0,\,T), \label{eq:dynamics}\\[1ex]
    &&&\mathComment{Path constraints with lower bounds, see section \ref{sec:constraints:path}}\nonumber\\
    %% path constraints with lower slack
    &&&\underline{h} \leq h(x(t), u(t), z(t), p(t)) + J_{\textrm{sh}} \, s\lowerh(t), &&\quad t \in (0,\,T),\\
    ...
    &&& h(x(t), u(t), z(t), p(t)) - J_{\textrm{sh}} \, s\upperh(t) \leq \overline{h}, &&\quad t \in (0,\,T),\\
    ...
    &&&\underline{h}\terminal \leq h\terminal(x(T), p(T)) + J\ind{sh}\terminal\, s\lowerh\terminal, &&\label{eq:constraints:terminal:lower_bounds:start}\\
    ...
    &&& h\terminal(x(T), p(T)) - J\ind{sh}\terminal\, s\ind{u,h}\terminal \leq \overline{h}\terminal, &&\label{eq:constraints:terminal:upper_bounds:start}\\
...
\item model parameters $ p: \R \rightarrow \in \R^{\np} $
...
\begin{align}
l(x, u, t, p) = \psi(y(x, u, z, t, p) - y_\mathrm{ref}, t, p), \qquad m(x, p) = \psi\terminal(y\terminal(x, t, p) - y_\mathrm{ref}\terminal, p),
\end{align}
