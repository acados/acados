...
% cost
    ...
    &&& J_{\textrm{bx}}\terminal\, x(T) - J\ind{sbx}\terminal\, s\ind{u,bx}\terminal \leq \overline{x}^{e}, &&\\ % TODO: add Jsbx_e?!
...
\begin{table}[ht!]
    ...
    \begin{tabular}{cccc}
        \toprule
        Term & Attribute/Property & Data type & Required \\ \midrule
        $J\ind{bx}$ & \code{AcadosConstraints.idxbx}    & double & \optional  \\
        $\overline{x} $  & \code{AcadosConstraints.ubx}     & double  & \optional   \\
        $\underline{x} $    & \code{AcadosConstraints.lbx}     & double  & \optional  \\[1em]
        $J\ind{bu}$ & \code{AcadosConstraints.idxbu}    & double   & \optional    \\
        $\overline{u} $         & \code{AcadosConstraints.ubu}     & double  & \optional   \\
        $\underline{u} $    & \code{AcadosConstraints.lbu}     & double   & \optional   \\ [1em]
        $C$ & \code{AcadosConstraints.C}    & double  & \optional \\
        $D $    & \code{AcadosConstraints.D}     & double  & \optional \\
        $\overline{g} $         & \code{AcadosConstraints.ug}     & double  & \optional   \\
        $\underline{g} $    & \code{AcadosConstraints.lg}     & double  & \optional    \\ [1em]
        $ h\initial $ & \code{AcadosModel.con\_h\_expr\_0}    & \casadi~expression  & \optional   \\
        $\overline{h}\initial $         & \code{AcadosConstraints.uh\_0}     & double  & \optional   \\
        $\underline{h}\initial $    & \code{AcadosConstraints.lh\_0}     & double   & \optional   \\ [1em]
        $ h $ & \code{AcadosModel.con\_h\_expr}    & \casadi~expression  & \optional   \\
        $\overline{h} $         & \code{AcadosConstraints.uh}     & double  & \optional   \\
        $\underline{h} $    & \code{AcadosConstraints.lh}     & double   & \optional   \\ [1em]
        $ J\ind{sbx} = \mathrm{diag}(\mathcal{I}\ind{sbx})$ & \code{AcadosConstraints.idxsbx} & double & \optional  \\
        $ J\ind{sbu} = \mathrm{diag}(\mathcal{I}\ind{sbu})$ & \code{AcadosConstraints.idxsbu} & double  & \optional \\
        $ J\ind{sg} = \mathrm{diag}(\mathcal{I}\ind{sg})$ & \code{AcadosConstraints.idxsg} & double & \optional  \\
        $ J\ind{sh} = \mathrm{diag}(\mathcal{I}\ind{sh})$ & \code{AcadosConstraints.idxsh} & double  & \optional  \\
        $ J\ind{sh}\initial = \mathrm{diag}(\mathcal{I}\ind{sh}\initial)$ & \code{AcadosConstraints.idxsh\_0} & double  & \optional  \\
        \bottomrule
    \end{tabular}
...
\begin{table}[ht!]
    ...
    \begin{tabular}{cccc}
        \toprule
        Term & Attribute/Property & Data type & Required \\ \midrule
        $J\ind{bx}\terminal  = \mathrm{diag}(\mathcal{I}\ind{bx}\terminal)$ & \code{AcadosConstraints.idxbx\_e}    & double   & \optional   \\
        $\overline{x}\terminal $         & \code{AcadosConstraints.ubx\_e}     & double   & \optional  \\
        $\underline{x}\terminal $    & \code{AcadosConstraints.lbx\_e}     & double  & \optional   \\ [1em]
        $ C\terminal $ & \code{AcadosConstraints.C\_e}    & double   & \optional   \\
        $\overline{g}\terminal $         & \code{AcadosConstraints.ug\_e}     & double   & \optional  \\
        $\underline{g}\terminal $    & \code{AcadosConstraints.lg\_e}     & double   & \optional   \\ [1em]
        $ h\terminal $ & \code{AcadosModel.con\_h\_expr\_e}    & \casadi~expression   & \optional  \\
        $\overline{h}\terminal $         & \code{AcadosConstraints.uh\_e}     & double  & \optional   \\
        $\underline{h}\terminal $    & \code{AcadosConstraints.lh\_e}     & double   & \optional   \\ [1em]
        $ J\ind{sbx}\terminal = \mathrm{diag}(\mathcal{I}\ind{sbx}\terminal)$ & \code{AcadosConstraints.idxsbx\_e} & double   & \optional \\
        $ J\ind{sg}\terminal = \mathrm{diag}(\mathcal{I}\ind{sg}\terminal)$ & \code{AcadosConstraints.idxsg\_e} & double & \optional  \\
        $ J\ind{sh}\terminal = \mathrm{diag}(\mathcal{I}\ind{sh}\terminal)$ & \code{AcadosConstraints.idxsh\_e} & double  & \optional  \\
        \bottomrule
    \end{tabular}
...
A full list of the available options is given on the \href{https://docs.acados.org/python_interface/index.html#acados_template.acados_ocp_options.AcadosOcpOptions}{docs page generated from the docstrings in~\python}.
