...
/* vdeFun */
int vde_chain_nm9(void* mem, const real_t** arg, real_t** res, int* iw, real_t* w) {
...
int vde_chain_nm9_init(int* n_in, int* n_out, int* n_int, int* n_real) {
  if (n_in) *n_in = 4;
  if (n_out) *n_out = 3;
  if (n_int) *n_int = 0;
  if (n_real) *n_real = 0;
...
int vde_chain_nm9_alloc(void** mem, const int* idata, const double* rdata) {
  if (mem) *mem = 0;
  (void)idata;
  (void)rdata;
  return 0;
...
int vde_chain_nm9_free(void* mem) {
  (void)mem;
  return 0;
}

int vde_chain_nm9_sparsity(int i, int *nrow, int *ncol, const int **colind, const int **row) {
  const int* s;
  switch (i) {
    case 0:
    case 4:
