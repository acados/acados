...
for (jj = 0; jj < nbu; jj++) {
    ...
    ************************************************/
    ...
    for (int ii = 0; ii < N+1; ii++) NS[ii] = 0;
    ocp_qp_dims tmpdims;
    tmpdims.N = N;
    tmpdims.nx = nxx;
    tmpdims.nu = nuu;
    ...
    form_nbu_nbx_rev(N, tmpdims.nbu, tmpdims.nbu, tmpdims.nb, tmpdims.nbx, tmpdims.nbu, hidxb);

    ocp_qp_in *new_qp_in = create_ocp_qp_in(&tmpdims);

    convert_colmaj_to_ocp_qp_in(&qp_in, new_qp_in);
    ...
    ************************************************/
    ...
    ocp_qp_out *qp_out = create_ocp_qp_out(new_qp_in->dim);

    /************************************************
    * ipm
    ************************************************/

    ocp_qp_hpipm_args *arg = ocp_qp_hpipm_create_arguments(new_qp_in->dim);

    arg->hpipm_args->iter_max = 10;

    ocp_qp_hpipm_memory *mem = ocp_qp_hpipm_create_memory(new_qp_in->dim, arg);
	...
	for (int rep = 0; rep < NREP; rep++) {
        acados_return = ocp_qp_hpipm(new_qp_in, qp_out, arg, mem);
	}
    ...
    ************************************************/
    ...
    ocp_qp_dims *dims = new_qp_in->dim;
    ...
    ************************************************/
    ...
    printf("\nu = \n");
    for (int ii = 0; ii < N; ii++) d_print_mat(1, nuu[ii], sol->u[ii], 1);

    printf("\nx = \n");
    for (int ii = 0; ii <= N; ii++) d_print_mat(1, nxx[ii], sol->x[ii], 1);

    printf("\npi = \n");
    for (int ii = 0; ii < N; ii++) d_print_mat(1, nxx[ii+1], sol->pi[ii], 1);

    printf("\nlam = \n");
    for (int ii = 0; ii <= N; ii++) d_print_mat(1, 2*nbb[ii]+2*ngg[ii], sol->lam[ii], 1);
