...
cost_y = SX.zeros(nx+nu) + [cost_x;cost_u];
cost_jac_y = SX.zeros(nx+nu,nx+nu) + jacobian(cost_y,cost_xu);
...
cost_yN = SX.zeros(nx) + cost_x;
cost_jac_yN = SX.zeros(nx,nx) + jacobian(cost_yN,cost_x);
...
pathcon_jac_g = SX.zeros(0,nx+nu) + jacobian(pathcon_g, cost_xu);
...
pathcon_jac_gN = SX.zeros(0,nx) + jacobian(pathcon_gN, cost_x);
