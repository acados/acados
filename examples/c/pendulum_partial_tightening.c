...
//           fprintf(gnuplotPipe, "set xlabel \"%s\"\n", "time [s]");
//           fprintf(gnuplotPipe, "plot \"%s\" with lines lt rgb \"blue\"\n",
//           x1_temp_file); fflush(gnuplotPipe); for (i=0; i < NSIM; i++) {
//               x = t_grid[i];
//               y = w[i*(NX+NU)];
//               fprintf(tempDataFile, "%lf %lf\n", x, y);
//           }
...
//           fprintf(gnuplotPipe, "set xlabel \"%s\"\n", "time [s]");
//           fprintf(gnuplotPipe, "plot \"%s\" with lines lt rgb \"blue\"\n",
//           x2_temp_file); fflush(gnuplotPipe); for (i=0; i < NSIM; i++) {
//               x = t_grid[i];
//               y = w[i*(NX+NU)+1];
//               fprintf(tempDataFile, "%lf %lf\n", x, y);
//           }
...
//           fprintf(gnuplotPipe, "set xlabel \"%s\"\n", "time [s]");
//           fprintf(gnuplotPipe, "plot \"%s\" with steps lt rgb \"red\" \n",
//           u1_temp_file); fflush(gnuplotPipe); for (i=0; i < NSIM; i++) {
//               x = t_grid[i];
//               y = w[i*(NX+NU)+4];
//               fprintf(tempDataFile, "%lf %lf\n", x, y);
//           }
...
extern int d_back_ric_rec_work_space_size_bytes_libstr(
    int N, int *nx, int *nu, int *nb,
    int *ng);  // TODO(Andrea): ask Gian how to fix this
...
// feenableexcept(FE_INVALID);
    ...
    // real_t  w_cl[NSIM*(NX+NU)]        = {0};  // States and controls stacked
    // closed loop real_t  pi_n[NN*(NX)]             = {0};
    ...
    //     sim_erk_create_arguments(&rk_opts, 4);
    //     void *erk_work;
    //     int_t erk_workspace_size = sim_erk_calculate_workspace_size(&sim_in,
    //     &rk_opts); erk_work = (void *) malloc(erk_workspace_size);
    ...
    for (int_t i = 0; i < N; i++) {
        ...
        for (int_t j = 0; j < 2 * (NBX + NBU); j++)
            lam_n[i * 2 * (NBX + NBU) + j] = lam_init;
        for (int_t j = 0; j < 2 * (NBX + NBU); j++)
            t_n[i * 2 * (NBX + NBU) + j] = t_init;
    ...
    for (int_t sim_iter = 0; sim_iter < NSIM; sim_iter++) {
        for (int_t iter = 0; iter < NREP; iter++) {
            ...
            // for ( int_t j = 0; j < NBU; j++ ) plb[0][j] = u_min[j] -
            // w[0*(NX+NU)+NX+j]; for ( int_t j = 0; j < NBU; j++ ) pub[0][j] =
            // u_max[j] - w[0*(NX+NU)+NX+j];
        ...
        for (int_t j = 0; j < 2 * (NBX + NBU); j++)
            lam_n[0 * 2 * (NBX + NBU) + j] = qp_out.lam[0][j] + GAMMA;
        for (int_t j = 0; j < 2 * (NBX + NBU); j++)
            t_n[0 * 2 * (NBX + NBU) + j] = qp_out.t[0][j] + GAMMA;
        ...
        for (int_t i = 1; i < N; i++) {
            ...
            // for (int_t j = 0; j < NX; j++) pi_n[0*NX+j] = qp_out.pi[0][j];
            for (int_t j = 0; j < 2 * (NBX + NBU); j++)
                lam_n[i * 2 * (NBX + NBU) + j] = qp_out.lam[i][j] + GAMMA;
            for (int_t j = 0; j < 2 * (NBX + NBU); j++)
                t_n[i * 2 * (NBX + NBU) + j] = qp_out.t[i][j] + GAMMA;
        }
        for (int_t j = 0; j < NX; j++) w[N * (NX + NU) + j] += qp_out.x[N][j];
        // for (int_t j = 0; j < NX; j++) pi_n[0*NX+j] = qp_out.pi[0][j];
        for (int_t j = 0; j < 2 * NBX; j++)
            lam_n[N * 2 * (NBX + NBU) + j] = qp_out.lam[N][j] + GAMMA;
        for (int_t j = 0; j < 2 * NBX; j++)
            t_n[N * 2 * (NBX + NBU) + j] = qp_out.t[N][j] + GAMMA;

        // for (int_t j = 0; j < NX; j++) w_cl[sim_iter*(NX+NU) + j] = w[j];
        // for (int_t j = 0; j < NU; j++) w_cl[sim_iter*(NX+NU) + NX + j] =
        // w[j+NX];
