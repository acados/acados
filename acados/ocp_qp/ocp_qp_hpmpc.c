...
// // work space size
// int ocp_qp_hpmpc_workspace_size_bytes(int N, int *nx, int *nu, int *nb, int
// *ng, int **hidxb,
...
//
//     int workspace_size = 8 + 5*k_max*sizeof(double);  // alignment to
//     single-word (4-byte)
//
...
//     int N2 = hpmpc_args->N2;  // horizon length of the partially condensed
//     problem int out_iter = -1;  // number of performed iterations double
//     *inf_norm_res = hpmpc_args->inf_norm_res;
//
//     // memory for stat
//     size_t addr = ( ( (size_t) workspace) + 7) / 8 * 8;  // align to 8-byte
//     boundaries double *ptr_double = (double *) addr; double *stat =
//     ptr_double; ptr_double += 5*k_max; for (ii = 0; ii < 5*k_max; ii++)
//     stat[ii] = 0.0;  // zero
//
//     // memory for idxb
//     int *hidxb[N+1];
// //  size_t addr = (( (size_t) workspace ) + 3 ) / 4 * 4;  // align to 4-byte
// boundaries
...
//       d_cvt_tran_mat2strmat(nx[ii+1], nu[ii], hB[ii], nx[ii+1], &hsBAbt[ii],
//       0, 0); d_cvt_tran_mat2strmat(nx[ii+1], nx[ii], hA[ii], nx[ii+1],
//       &hsBAbt[ii], nu[ii], 0); d_cvt_tran_mat2strmat(nx[ii+1], 1, hb[ii],
//       nx[ii+1], &hsBAbt[ii], nu[ii]+nx[ii], 0);
//
//       d_create_strvec(nx[ii+1], &hsb[ii], ptr_memory);
//       ptr_memory += (&hsb[ii])->memory_size;
//       d_cvt_vec2strvec(nx[ii+1], hb[ii], &hsb[ii], 0);
//
//       d_create_strmat(nu[ii]+nx[ii]+1, nu[ii]+nx[ii], &hsRSQrq[ii],
//       ptr_memory); ptr_memory += (&hsRSQrq[ii])->memory_size;
//       d_cvt_mat2strmat(nu[ii], nu[ii], hR[ii], nu[ii], &hsRSQrq[ii], 0, 0);
//       d_cvt_tran_mat2strmat(nu[ii], nx[ii], hS[ii], nu[ii], &hsRSQrq[ii],
//       nu[ii], 0); d_cvt_mat2strmat(nx[ii], nx[ii], hQ[ii], nx[ii],
//       &hsRSQrq[ii], nu[ii], nu[ii]); d_cvt_tran_mat2strmat(nu[ii], 1, hr[ii],
//       nu[ii], &hsRSQrq[ii], nu[ii]+nx[ii], 0); d_cvt_tran_mat2strmat(nx[ii],
//       1, hq[ii], nx[ii], &hsRSQrq[ii], nu[ii]+nx[ii], nu[ii]);
//
...
//       d_cvt_tran_mat2strmat(ng[ii], nu[ii], hD[ii], ng[ii], &hsDCt[ii], 0,
//       0); d_cvt_tran_mat2strmat(ng[ii], nx[ii], hC[ii], ng[ii], &hsDCt[ii],
//       nu[ii], 0);
//
...
//     nu[ii], 0); d_cvt_mat2strmat(nx[ii], nx[ii], hQ[ii], nx[ii],
//     &hsRSQrq[ii], nu[ii], nu[ii]); d_cvt_tran_mat2strmat(nu[ii], 1, hr[ii],
//     nu[ii], &hsRSQrq[ii], nu[ii]+nx[ii], 0); d_cvt_tran_mat2strmat(nx[ii], 1,
//     hq[ii], nx[ii], &hsRSQrq[ii], nu[ii]+nx[ii], nu[ii]);
//
//     d_create_strvec(2*nb[ii]+2*ng[ii], &hsd[ii], ptr_memory);
//     ptr_memory += (&hsd[ii])->memory_size;
//     d_cvt_vec2strvec(nb[ii], hlb[ii], &hsd[ii], 0);
...
//     double **hx = qp_out->x;
//     double **hu = qp_out->u;
//     // double **hpi = qp_out->pi; //TODO(Andrea): multiplers not returned at
//     the moment
//     // double **hlam = qp_out->lam; //TODO(Andrea): multiplers not returned
//     at the moment
//
//     // extract args struct members
//     double mu_tol = hpmpc_args->tol;
...
//       // d_ip2_res_mpc_hard_tv_work_space_size_bytes_libstr(N, nx, nu, nb,
//       ng));
//
...
int ocp_qp_hpmpc(ocp_qp_in *qp_in, ocp_qp_out *qp_out, void *args_, void *mem_,
    ...
    if (M < N) {  // XXX andrea partial tightening stuff
        ...
        for (ii = 0; ii < N; ii++) {
            d_create_strmat(nu[ii] + nx[ii] + 1, nx[ii + 1], &hsBAbt[ii],
                            ptr_memory);
            ptr_memory += (&hsBAbt[ii])->memory_size;
            d_cvt_tran_mat2strmat(nx[ii + 1], nu[ii], hB[ii], nx[ii + 1],
                                  &hsBAbt[ii], 0, 0);
            d_cvt_tran_mat2strmat(nx[ii + 1], nx[ii], hA[ii], nx[ii + 1],
                                  &hsBAbt[ii], nu[ii], 0);
            d_cvt_tran_mat2strmat(nx[ii + 1], 1, hb[ii], nx[ii + 1],
                                  &hsBAbt[ii], nu[ii] + nx[ii], 0);
            ...
            d_cvt_mat2strmat(nu[ii], nu[ii], hR[ii], nu[ii], &hsRSQrq[ii], 0,
                             0);
            d_cvt_tran_mat2strmat(nu[ii], nx[ii], hS[ii], nu[ii], &hsRSQrq[ii],
                                  nu[ii], 0);
            d_cvt_mat2strmat(nx[ii], nx[ii], hQ[ii], nx[ii], &hsRSQrq[ii],
                             nu[ii], nu[ii]);
            d_cvt_tran_mat2strmat(nu[ii], 1, hr[ii], nu[ii], &hsRSQrq[ii],
                                  nu[ii] + nx[ii], 0);
            d_cvt_tran_mat2strmat(nx[ii], 1, hq[ii], nx[ii], &hsRSQrq[ii],
                                  nu[ii] + nx[ii], nu[ii]);
            ...
            d_create_strmat(nu[ii] + nx[ii] + 1, ng[ii], &hsDCt[ii],
                            ptr_memory);
            ptr_memory += (&hsDCt[ii])->memory_size;
            d_cvt_tran_mat2strmat(ng[ii], nu[ii], hD[ii], ng[ii], &hsDCt[ii], 0,
                                  0);
            d_cvt_tran_mat2strmat(ng[ii], nx[ii], hC[ii], ng[ii], &hsDCt[ii],
                                  nu[ii], 0);
            ...
            d_cvt_vec2strvec(nu[ii] + nx[ii], hpmpc_args->ux0[ii], &hsdux[ii],
                             0);
            ptr_memory += (&hsdux[ii])->memory_size;
            d_create_strvec(nu[ii] + nx[ii], &hsux[ii], ptr_memory);
            d_cvt_vec2strvec(nu[ii] + nx[ii], hpmpc_args->ux0[ii], &hsux[ii],
                             0);
            ptr_memory += (&hsux[ii])->memory_size;
            ...
            d_cvt_vec2strvec(2 * nb[ii] + 2 * ng[ii], hpmpc_args->lam0[ii],
                             &hslam[ii], 0);
            ptr_memory += (&hslam[ii])->memory_size;

            d_create_strvec(2 * nb[ii] + 2 * ng[ii], &hst[ii], ptr_memory);
            // copy slacks from hpmpc_args
            d_cvt_vec2strvec(2 * nb[ii] + 2 * ng[ii], hpmpc_args->t0[ii],
                             &hst[ii], 0);
            ptr_memory += (&hst[ii])->memory_size;

            d_create_strvec(nx[ii + 1], &hsPb[ii + 1], ptr_memory);
            ptr_memory += (&hsPb[ii + 1])->memory_size;
            d_create_strmat(nu[ii] + nx[ii] + 1, nu[ii] + nx[ii], &hsL[ii],
                            ptr_memory);
        ...
        d_create_strmat(nu[ii] + nx[ii] + 1, nu[ii] + nx[ii], &hsRSQrq[ii],
                        ptr_memory);
        ptr_memory += (&hsRSQrq[ii])->memory_size;
        d_cvt_mat2strmat(nx[ii], nx[ii], hQ[ii], nx[ii], &hsRSQrq[ii], nu[ii],
                         nu[ii]);
        d_cvt_tran_mat2strmat(nx[ii], 1, hq[ii], nx[ii], &hsRSQrq[ii],
                              nu[ii] + nx[ii], nu[ii]);
        ...
        d_create_strvec(2 * nb[ii] + 2 * ng[ii], &hst[ii], ptr_memory);
        d_cvt_vec2strvec(2 * nb[ii] + 2 * ng[ii], hpmpc_args->t0[ii], &hst[ii],
                         0);
        ptr_memory += (&hst[ii])->memory_size;
        ...
        work_ric = ptr_memory;
        ptr_memory +=
            d_back_ric_rec_work_space_size_bytes_libstr(N, nx, nu, nb, ng);

        // update cost function matrices and vectors (box constraints)
        d_update_hessian_gradient_mpc_hard_libstr(
            N - M, &nx[M], &nu[M], &nb[M], &ng[M], &hsd[M], sigma_mu, &hst[M],
            &hstinv[M], &hslam[M], &hslamt[M], &hsdlam[M], &hsQx[M], &hsqx[M]);

        // backward riccati factorization and solution at the end
        d_back_ric_rec_sv_back_libstr(
            N - M, &nx[M], &nu[M], &nb[M], &hidxb[M], &ng[M], 0, &hsBAbt[M],
            hsvecdummy, 1, &hsRSQrq[M], &hsrq[M], &hsDCt[M], &hsQx[M], &hsqx[M],
            &hsux[M], 1, &hspi[M], 1, &hsPb[M], &hsL[M], work_ric);
        ...
        // recover [P p; p' *]
        dsyrk_ln_mn_libstr(nx[M] + 1, nx[M], nx[M], 1.0, &sLxM, 0, 0, &sLxM, 0,
                           0, 0.0, &sPpM, 0, 0, &sPpM, 0, 0);
        ...
        hpmpc_status = d_ip2_res_mpc_hard_libstr(
            &kk, k_max, mu0, mu_tol, alpha_min, warm_start, stat, M, nx, nu, nb,
            hidxb, ng, hsBAbt, hsRSQrq, hsDCt, hsd, hsux, compute_mult, hspi,
            hslam, hst, ptr_memory);  // recover original stage M
        ...
        d_back_ric_rec_sv_forw_libstr(N - M, &nx[M], &nu[M], &nb[M], &hidxb[M],
                                      &ng[M], 0, &hsBAbt[M], hsvecdummy, 1,
                                      &hsRSQrq[M], &hsrq[M], hsmatdummy,
                                      &hsQx[M], &hsqx[M], &hsux[M], 1, &hspi[M],
                                      1, &hsPb[M], &hsL[M], hsric_work_mat);
        ...
        d_compute_alpha_mpc_hard_libstr(
            N - M, &nx[M], &nu[M], &nb[M], &hidxb[M], &ng[M], &alpha, &hst[M],
            &hsdt[M], &hslam[M], &hsdlam[M], &hslamt[M], &hsdux[M], &hsDCt[M],
            &hsd[M]);

        // overwrite alpha (taking full steps and performing line-search in
        // out_iter level)
        ...
        d_update_var_mpc_hard_libstr(N - M, &nx[M], &nu[M], &nb[M], &ng[M],
                                     &sigma_mu, mu_scal, alpha, &hsux[M],
                                     &hsdux[M], &hst[M], &hsdt[M], &hslam[M],
                                     &hsdlam[M], &hspi[M], &hspi[M]);

        // !!!! TODO(Andrea): equality multipliers are not being updated! Need
        // to define and compute hsdpi (see function prototype).
        ...
        int warm_start = hpmpc_args->warm_start;
        int N2 = hpmpc_args
                     ->N2;  // horizon length of the partially condensed problem
        int out_iter = -1;  // number of performed iterations
        double *inf_norm_res = hpmpc_args->inf_norm_res;

        // memory for stat
        size_t addr =
            (((size_t)workspace_) + 7) / 8 * 8;  // align to 8-byte boundaries
        ...
        hpmpc_status = fortran_order_d_ip_ocp_hard_tv(
            &out_iter, k_max, mu0, mu_tol, N, nx, nu, nb, hidxb, ng, N2,
            warm_start, hA, hB, hb, hQ, hS, hR, hq, hr, hlb, hub, hC, hD, hlg,
...
int_t ocp_qp_hpmpc_calculate_workspace_size(ocp_qp_in *qp_in, void *args_) {
    ...
    if (M < N) {  // XXX andrea partial tightening stuff
        ...
        ws_size += hpmpc_d_ip_ocp_hard_tv_work_space_size_bytes(N, nx, nu, nb,
                                                                hidxb, ng, N2);
    }
    return ws_size;
}
...
int_t ocp_qp_hpmpc_create_memory(ocp_qp_in *in, void *args_, void **mem_) {
    ...
    if (M < N) {  // XXX andrea partial tightened stuff
        ...
        for (ii = 0; ii < N; ii++) {
            ...
            mem_size +=
                d_size_strmat(nu[ii] + nx[ii] + 1, nu[ii] + nx[ii]);  // RSQrq
...
//     // hidxb, ng, N2, warm_start, hA, hB, hb, hQ, hS, hR, hq, hr, hlb, hub,
//     hC, hD, hlg, hug,
//     // hx, hu, hpi, hlam, ht, inf_norm_res, workspace, stat, ux0, pi0, lam0,
//     t0);
//
//     if (hpmpc_status == 1) acados_status = ACADOS_MAXITER;
//
