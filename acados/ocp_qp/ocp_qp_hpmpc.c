...
// TODO(dimitris): TEMPORARY FUNCTIONS! SHOULD UPDATE HPMPC INSTEAD

void tmp_d_part_cond_compute_problem_size(int N, int *nx, int *nu, int *nb, int *nbu, int *nbx, int *ng, int N2, int *nx2, int *nu2, int *nb2, int *ng2)
	{
...
int tmp_hpmpc_d_ip_ocp_hard_tv_work_space_size_bytes(int N, int *nx, int *nu, int *nb, int *nbu, int *nbx, int *ng, int N2)
{
	...
	if(N2<N) // partial condensing
		...
		tmp_d_part_cond_compute_problem_size(N, nx, nu, nb, nbu, nbx, ng, N2, nx2, nu2, nb2, ng2);
		...
		// partial condensing
		size += d_part_cond_memory_space_size_bytes(N, nx, nu, nb, NULL, ng, N2, nx2, nu2, nb2, ng2);
		size += d_part_cond_work_space_size_bytes(N, nx, nu, nb, NULL, ng, N2, nx2, nu2, nb2, ng2);
...
int ocp_qp_hpmpc_calculate_memory_size(ocp_qp_dims *dims, void *args_)
{
    ...
    ws_size += tmp_hpmpc_d_ip_ocp_hard_tv_work_space_size_bytes(N, nx, nu, nb, dims->nbu, dims->nbx, ng, N2);
