...
//	double alpha = hpmpc_args->min_step; // fixed in the solver
	...
	int hpmpc_status = fortran_order_d_ip_ocp_hard_tv(&kk, k_max, mu0, mu_tol, N, nx, nu, nb, hidxb, ng, N2, warm_start, hA, hB, hb, hQ, hS, hR, hq, hr, hlb, hub, hC, hD, hlg, hug, hx, hu, hpi, hlam, inf_norm_res, workspace, stat);
...
#if 0
    printf("\n");
    printf("%e %e %e %e", inf_norm_res[0], inf_norm_res[1], inf_norm_res[2], inf_norm_res[3]);
    printf("\n");
    for (ii = 0; ii < kk; ii++)
