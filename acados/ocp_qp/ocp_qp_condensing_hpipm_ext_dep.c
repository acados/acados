...
ocp_qp_condensing_hpipm_memory *ocp_qp_condensing_hpipm_create_memory(ocp_qp_in *qp_in, void *args_) {
    ...
    int memory_size = ocp_qp_condensing_hpipm_calculate_memory_size(qp_in->size, args);
    void *ptr = malloc(memory_size);
    char *ptr_end = ocp_qp_condensing_hpipm_assign_memory(qp_in, args, (void **) &mem, ptr);
