    ...
    for (istep = 0; istep < num_steps; istep++)
	{
        ...
        for (s = 0; s < ns; s++)
		{
			...
			if (opts->sens_forw) // simulation + forward sensitivities
			{
				...
				model->expl_vde_for->evaluate(model->expl_vde_for, ext_fun_in, ext_fun_out);  // forward VDE evaluation
			}
			else // simulation only
			{
				...
				model->expl_ode_fun->evaluate(model->expl_ode_fun, ext_fun_in, ext_fun_out);  // ODE evaluation
			}
        ...
        for (istep = num_steps - 1; istep > -1; istep--)
		{
            ...
            for (s = ns - 1; s > -1; s--)
			{
                ...
                if (opts->sens_hess)
				{
                    ...
                    model->expl_ode_hes->evaluate(model->expl_ode_hes, ext_fun_in, ext_fun_out);
                }
				else
				{
					ext_fun_in[0] = rhs_adj_in+0; // x: nx
                    ...
                    model->expl_vde_adj->evaluate(model->expl_vde_adj, ext_fun_in, ext_fun_out); // adjoint VDE evaluation
                }
                timing_ad += acados_toc(&timer_ad);
