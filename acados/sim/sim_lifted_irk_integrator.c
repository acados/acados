...
int sim_lifted_irk_workspace_calculate_size(void *config_, sim_dims *dims, void *opts_)
{
    ...
    if (opts->scheme->type == simplified_in ||
        opts->scheme->type == simplified_inis) {
        size +=
            ((ns * nx) * (1 + NF)) * sizeof(double);  // sys_sol_trans
...
static void form_linear_system_matrix(void *config_, int istep, const sim_in *in, void *opts_, sim_lifted_irk_memory *mem, sim_lifted_irk_workspace *work, double *sys_mat, double **sys_mat2, double timing_ad)
{
	...
	double *ext_fun_in[5]; // XXX large enough ?
	double *ext_fun_out[5]; // XXX large enough ?
    ...
    if ((opts->scheme->type == simplified_in ||
        ...
        for (i = 0; i < ns * nx * ns * nx; i++)
            sys_mat[i] = 0.0;
        for (i = 0; i < ns * nx; i++)
            sys_mat[i * (ns * nx + 1)] = 1.0;  // identity
    }
    ...
    for (s1 = 0; s1 < ns; s1++) {
        ...
        for (s2 = 0; s2 < ns; s2++) {
            for (i = 0; i < nx; i++) {
                rhs_in[i] += H_INT * A_mat[s2 * ns + s1] *
                             K_traj[istep * ns * nx + s2 * nx + i];
            }
        }
        acados_tic(&timer_ad);
        ...
        model->expl_ode_jac->evaluate(model->expl_ode_jac, ext_fun_in, ext_fun_out);  // k evaluation
        ...
        if (opts->scheme->type == simplified_in ||
            opts->scheme->type == simplified_inis) {
            for (i = 0; i < nx * nx; i++)
                jac_traj[istep * ns + s1][i] = jac_tmp[nx + i];
        }
        ...
        if (opts->scheme->type == exact) {
            for (s2 = 0; s2 < ns; s2++) {
                for (j = 0; j < nx; j++) {
                    for (i = 0; i < nx; i++) {
                        sys_mat[(s2 * nx + j) * ns * nx + s1 * nx +
                                i] -= H_INT * A_mat[s2 * ns + s1] *
                                      jac_tmp[nx + j * nx + i];
                    }
                }
            }
        ...
        for (istep = opts->num_steps - 1; istep > -1; istep--) {  // ADJOINT update
            ...
            if (opts->scheme->type == simplified_inis && !opts->scheme->freeze) {
                for (s1 = 0; s1 < ns; s1++) {
                    for (j = 0; j < NF; j++) {
                        for (i = 0; i < nx; i++) {
                            DK_traj[(istep * ns + s1) * nx * NF +
                                    j * nx + i] +=
                                delta_DK_traj[(istep * ns + s1) * nx *
                                                  NF +
                                              j * nx + i];
                        }
                    }
                ...
                if (opts->scheme->type == simplified_in ||
                    ...
                    for (s1 = 0; s1 < ns; s1++) {
                        for (s2 = 0; s2 < ns; s2++) {
                            work->trans[s2 * ns + s1] =
                                1.0 / H_INT *
                                opts->scheme->transf1_T[s2 * ns + s1];
                        }
                    }
                    ...
                    construct_subsystems(sys_sol_trans, sys_sol2, ns,
                                         nx, 1);
                }
                acados_tic(&timer_la);
                int idx = 0;
...
#error : NOT YET IMPLEMENTED
                ...
                if (opts->scheme->type == simplified_in ||
                    ...
                    destruct_subsystems(sys_sol_trans, sys_sol2, ns, nx,
                                        1);
...
// ---- BLASFEO: LU factorization ----
        ...
        for (s1 = 0; s1 < ns; s1++) {
            ...
            model->expl_vde_for->evaluate(model->expl_vde_for, ext_fun_in, ext_fun_out);  // k evaluation
            timing_ad += acados_toc(&timer_ad);

            // put VDE_tmp in sys_sol:
            for (j = 0; j < 1 + NF; j++) {
                for (i = 0; i < nx; i++) {
                    sys_sol[j * ns * nx + s1 * nx + i] =
                        VDE_tmp[s1][j * nx + i];
                }
            }
            for (i = 0; i < nx; i++) {
                sys_sol[s1 * nx + i] -=
                    K_traj[istep * ns * nx + s1 * nx + i];
            }
            if (opts->scheme->type == simplified_inis) {
                for (j = 0; j < NF; j++) {
                    for (i = 0; i < nx; i++) {
        ...
        if (opts->scheme->type == simplified_inis) {
            for (j = 0; j < NF; j++) {
                for (i = 0; i < ns * nx; i++) {
                    out->grad[j] += mu_traj[istep * ns * nx + i] *
                                    sys_sol[(j + 1) * ns * nx + i];
                }
            }
        }
        ...
        if (opts->scheme->type == simplified_in ||
            ...
            for (s1 = 0; s1 < ns; s1++) {
                for (s2 = 0; s2 < ns; s2++) {
                    work->trans[s2 * ns + s1] =
                        1.0 / H_INT *
                        opts->scheme->transf1[s2 * ns + s1];
                }
            }
            if (!opts->scheme->freeze) {
                transform_mat(sys_sol, work->trans, sys_sol_trans, ns,
                              nx, 1 + NF);
            } else {
                transform_mat(sys_sol, work->trans, sys_sol_trans, ns,
                              nx, 1);
            ...
            // construct sys_sol2 from sys_sol_trans:
            if (!opts->scheme->freeze) {
                construct_subsystems(sys_sol_trans, sys_sol2, ns, nx,
                                     1 + NF);
            } else {
                construct_subsystems(sys_sol_trans, sys_sol2, ns, nx,
                                     1);
        ...
        for (s1 = 0; s1 < ns; s1++) {
            for (i = 0; i < nx; i++) {
                out_tmp[i] +=
                    H_INT * b_vec[s1] *
                    K_traj[istep * ns * nx + s1 * nx + i];  // RK step
            }
        }
            ...
            for (j = 0; j < NF; j++) {
                for (i = 0; i < nx; i++) {
                    if (opts->scheme->type == simplified_inis && opts->sens_adj &&
                        !opts->scheme->freeze) {
                        delta_DK_traj[(istep * ns + s1) * nx * NF +
                                      j * nx + i] =
                            sys_sol[(j + 1) * ns * nx + s1 * nx + i];
                    } else if (opts->scheme->type == simplified_inis &&
                               !opts->scheme->freeze) {
                        DK_traj[(istep * ns + s1) * nx * NF + j * nx +
                                i] +=
                            sys_sol[(j + 1) * ns * nx + s1 * nx + i];
