...
int_t ocp_nlp_gn_sqp(const ocp_nlp_in *nlp_in, ocp_nlp_out *nlp_out,
    ...
    for (int_t sqp_iter = 0; sqp_iter < gn_sqp_args->common->maxIter;
         sqp_iter++) {
...
#pragma omp parallel for
        ...
        for (int_t i = 0; i < N; i++) {
            ...
            for (int_t j = 0; j < nu[i]; j++)
                sim[i].in->u[j] = w[w_idx + nx[i] + j];
            sim[i].in->sens_adj = (opts->scheme.type != exact && sqp_iter > 0);
            if (nlp_in->freezeSens) {  // freeze inexact sensitivities after
                                       // first SQP iteration !!
            ...
            for (int_t j = 0; j < nx[i]; j++) {
                ...
                if (fabs(qp_b[i][j]) > feas) feas = fabs(qp_b[i][j]);
                for (int_t k = 0; k < nx[i]; k++)
                    qp_A[i][j * nx[i] + k] =
                        sim[i].out->S_forw[j * nx[i] + k];  // COLUMN MAJOR
            }
            for (int_t j = 0; j < nu[i]; j++)
                for (int_t k = 0; k < nx[i]; k++)
                    qp_B[i][j * nx[i] + k] =
                        sim[i].out->S_forw[(nx[i] + j) * nx[i] +
                                           k];  // COLUMN MAJOR
        ...
        for (int_t i = 0; i < N + 1; i++) {
            ...
            for (int_t j = 0; j < nlp_in->nb[i]; j++) {
                if (nlp_in->idxb[i][j] < nu[i]) {
                    qp_lb[i][j] = nlp_in->lb[i][j] -
                                  w[w_idx + nx[i] + nlp_in->idxb[i][j]];
                    qp_ub[i][j] = nlp_in->ub[i][j] -
                                  w[w_idx + nx[i] + nlp_in->idxb[i][j]];
                } else {
                    qp_lb[i][j] = nlp_in->lb[i][j] -
                                  w[w_idx - nu[i] + nlp_in->idxb[i][j]];
                    qp_ub[i][j] = nlp_in->ub[i][j] -
                                  w[w_idx - nu[i] + nlp_in->idxb[i][j]];
                }
            }
            //    print_matrix_name((char*)"stdout", (char*)"qp_lb: ",
            //    gn_sqp_mem->qp_solver->qp_in->lb[i], 1,
            //    gn_sqp_mem->qp_solver->qp_in->nb[i]);
            //    print_matrix_name((char*)"stdout", (char*)"qp_ub: ",
            //    gn_sqp_mem->qp_solver->qp_in->ub[i], 1,
            //    gn_sqp_mem->qp_solver->qp_in->nb[i]);

            //    print_matrix_name((char*)"stdout", (char*)"nlp_lb: ",
            //    nlp_in->lb[i], 1, nlp_in->nb[i]);
            //    print_matrix_name((char*)"stdout", (char*)"nlp_ub: ",
            //    nlp_in->ub[i], 1, nlp_in->nb[i]);

            // Update gradients
            // TODO(rien): only for diagonal Q, R matrices atm
            // TODO(rien): only for least squares cost with state and control
            // reference atm
            if (i < N) {
                sim_RK_opts *opts = (sim_RK_opts *)sim[i].args;
                for (int_t j = 0; j < nx[i]; j++) {
                    qp_q[i][j] = cost->W[i][j * (nx[i] + nu[i] + 1)] *
                                 (w[w_idx + j] - y_ref[i][j]);
...
//        print_matrix_name((char*)"stdout", (char*)"qp_lb[N]", qp_lb[N], 1,
//        nx[N]); print_matrix_name((char*)"stdout", (char*)"qp_ub[N]",
//        qp_ub[N], 1, nx[N]);

//    gn_sqp_mem->qp_solver->initialize(gn_sqp_mem->qp_solver->qp_in,
//     gn_sqp_mem->qp_solver->args, gn_sqp_mem->qp_solver->mem,
//     gn_sqp_mem->qp_solver->work);
#ifdef DEBUG
        print_ocp_qp(gn_sqp_mem->qp_solver->qp_in);
#endif
        status = gn_sqp_mem->qp_solver->fun(
            gn_sqp_mem->qp_solver->qp_in, gn_sqp_mem->qp_solver->qp_out,
            gn_sqp_mem->qp_solver->args, gn_sqp_mem->qp_solver->mem,
            gn_sqp_mem->qp_solver->work);
        if (status) {
        ...
        for (int_t i = 0; i < N; i++) {
            ...
            w_idx += nx[i] + nu[i];
            //    print_matrix_name((char*)"stdout",
            //    (char*)"solver->qp_out->x[i]: ",
            //  gn_sqp_mem->qp_solver->qp_out->x[i], 1, nx[i]);
            //   print_matrix_name((char*)"stdout",
            //   (char*)"solver->qp_out->u[i]: ",
        ...
        fprintf(stdout,
                "--- ITERATION %d, Infeasibility: %+.3e , step X: %+.3e, "
                "step U: %+.3e \n",
