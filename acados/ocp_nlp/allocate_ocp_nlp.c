...
//     nlp->dims = malloc(sizeof(ocp_nlp_dims));

//     int_zeros((int_t **)&nlp->dims->nx, N + 1, 1);
//     int_zeros((int_t **)&nlp->dims->nu, N + 1, 1);
//     int_zeros((int_t **)&nlp->dims->nb, N + 1, 1);
//     int_zeros((int_t **)&nlp->dims->ng, N + 1, 1);
//     int_zeros((int_t **)&nlp->dims->nh, N + 1, 1);

//     nlp->dims->N = N;
//     memcpy((void *)nlp->dims->nx, (void *)nx, sizeof(*nx) * (N + 1));
//     memcpy((void *)nlp->dims->nu, (void *)nu, sizeof(*nu) * (N + 1));
// }

// static void free_ocp_nlp_in_basic(ocp_nlp_in *const nlp) {
//     int_free((int_t *)nlp->dims->nx);
//     int_free((int_t *)nlp->dims->nu);
//     int_free((int_t *)nlp->dims->nb);
//     int_free((int_t *)nlp->dims->ng);
//     int_free((int_t *)nlp->dims->nh);
// }
...
//     memcpy((void *)nlp->dims->nb, (void *)nb, sizeof(*nb) * (N + 1));
...
//     nlp->dims->N = N;
//     memcpy((void *)nlp->dims->ng, (void *)nc, sizeof(*nc) * (N + 1));

//     for (int_t i = 0; i <= N; i++) {
//         d_zeros((real_t **)&nlp->lg[i], nc[i], 1);
//         d_zeros((real_t **)&nlp->ug[i], nc[i], 1);
//         d_zeros((real_t **)&nlp->Cx[i], nc[i], nlp->dims->nx[i]);
//         d_zeros((real_t **)&nlp->Cu[i], nc[i], nlp->dims->nu[i]);
//     }
// }
...
//     nlp->lh = (const real_t **)calloc(N + 1, sizeof(*nlp->lh));
//     nlp->uh = (const real_t **)calloc(N + 1, sizeof(*nlp->uh));

//     nlp->dims->N = N;
//     memcpy((void *)nlp->dims->nh, (void *)ng, sizeof(*ng) * (N + 1));
//     for (int_t i = 0; i <= N; i++) {
//         d_zeros((real_t **)&nlp->lh[i], ng[i], 1);
//         d_zeros((real_t **)&nlp->uh[i], ng[i], 1);
//     }
// }

// static void free_ocp_nlp_in_nonlinear_constraints(ocp_nlp_in *const nlp) {
//     for (int_t i = 0; i <= nlp->dims->N; i++) {
//         d_free((real_t *)nlp->lh[i]);
//         d_free((real_t *)nlp->uh[i]);
//     }
//     free((real_t **)nlp->lh);
//     free((real_t **)nlp->uh);
// }
...
//     for (int_t k = 0; k < N; k++) {
//         d_zeros(&out->x[k], in->dims->nx[k], 1);
//         d_zeros(&out->u[k], in->dims->nu[k], 1);
//         d_zeros(&out->pi[k], in->dims->nx[k+1], 1);
//         d_zeros(&out->lam[k], 2 * (in->dims->nb[k] + in->dims->ng[k] + in->dims->nh[k]), 1);
//     }
//     d_zeros(&out->x[N], in->dims->nx[N], 1);
//     d_zeros(&out->u[N], in->dims->nu[N], 1);
//     d_zeros(&out->lam[N], 2 * (in->dims->nb[N] + in->dims->ng[N] + in->dims->nh[N]), 1);
// }
